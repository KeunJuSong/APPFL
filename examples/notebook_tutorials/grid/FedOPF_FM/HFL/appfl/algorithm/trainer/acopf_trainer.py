...
class ACOPFTrainer(BaseTrainer):
    ...
    def train(self, **kwargs):
        ...
        if self.train_configs.mode == "epoch":            
            ...
            # Global-group correction parameters (HFL)
            gg_correc_params = kwargs["gg_corr_params"].to(self.device)
            if self.round <= warmup_iter:
                ...
                for epoch in range(self.train_configs.num_local_epochs):
                    ...
                    for data in self.train_dataloader:
                        loss, pred = self._train_batch(optimizer, data, 
                                                            ...
                                                            LagM_sp_gen, LagM_gen, LagM_bus, LagM_line, gg_correc_params)                        
                        # train_loss += loss
                ...
                for epoch in range(self.train_configs.num_local_epochs):
                    ...
                    if (self.round)%p_iter_max_sum == 0:
                        ...
                        rho = rho_init * (1/(1+beta*(rho_iter + 1)))
                        rho_iter += 1
                        # NOTE: The code was wrong, but lets find which way would be better.
                        with torch.no_grad():
                            self.model.eval()
                            for data in self.train_dataloader:
    ...
    def _train_batch(
        ...
        for key, param in self.model.named_parameters():
            if 'flatten' not in key:
                if not isinstance(local_par_list, torch.Tensor):
                # Initially nothing to concatenate
                    local_par_list = param.reshape(-1)
                else:
                    local_par_list = torch.cat((local_par_list, param.reshape(-1)), 0)
        # loss_algo = (local_par_list * group_global_corr).abs().sum()
        loss_algo = (local_par_list * global_group_correc_params).square().sum()
        loss = loss + loss_algo
        loss.sum().backward()
