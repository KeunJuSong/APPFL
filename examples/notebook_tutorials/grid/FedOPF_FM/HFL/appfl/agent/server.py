...
class ServerAgent:
    ...
    def save_checkpoint(self, server_id, checkpoint_path: Optional[str] = None) -> None:
        """Save the model to a checkpoint file."""
        if checkpoint_path is None:
            output_dir = self.server_agent_config.server_configs.get(
                "checkpoint_dirname", "./output"
            )
            output_filename = self.server_agent_config.server_configs.get(
                "checkpoint_filename", "checkpoint"
            )
            # curr_time_str = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            if server_id == -1:
                checkpoint_path = (
                    f"{output_dir}/{output_filename}_Global.pth"
                )
            else:
                checkpoint_path = (
                    f"{output_dir}/{output_filename}_Server_{str(server_id)}.pth"
                )
    ...
    def save_data_tracking(self, metrics, checkpoint_path: Optional[str] = None) -> None:
        """Save the data tracking"""
        if checkpoint_path is None:
            output_dir = self.server_agent_config.server_configs.get(
                "checkpoint_dirname", "./output"
            )
            checkpoint_path = (
                f"{output_dir}/metrics.pickle"
            )
        # Make sure the directory exists
        if not os.path.exists(os.path.dirname(checkpoint_path)):
            pathlib.Path(os.path.dirname(checkpoint_path)).mkdir(
        ...
        with open(os.path.join(checkpoint_path), 'wb') as handle:
