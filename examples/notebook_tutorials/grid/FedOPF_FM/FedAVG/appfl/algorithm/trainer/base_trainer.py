...
class BaseTrainer:
    ...
    def load_parameters(
        ...
        """Load model parameters. NOTE that this is for FedOPF"""
        target_state_dict = self.model.state_dict()
        layer_names = list(target_state_dict.keys())
        for name in layer_names[:-2]:
            if name in target_state_dict.keys() and name in params.keys():
                target_state_dict[name] = params[name].clone()

        self.model.load_state_dict(target_state_dict, strict=False)
