...
class ACOPFProblem:
    ...
    def __init__(self, filename, train_frac_semi = 0.0833, valid_frac=0.0833*2, test_frac=0):
        ...
        ppc = loadcase("./data/matpower/pglib_opf_case5658_epigrids.mat")
        ppc = ext2int(ppc)
    ...
    def obj_fn(self, Y):
        ...
        pg_mw = pg * torch.tensor(self.genbase).to(self._device)
    ...
    def ineq_resid(self, X, Y):
        ...
        If = torch.tensor(Yf.todense(), dtype=torch.complex64).to(self._device) @ vz.T
        It = torch.tensor(Yt.todense(), dtype=torch.complex64).to(self._device) @ vz.T
    ...
    def eq_jac(self, Y):
        ...
        dreal_dpg = torch.zeros(self.nbus, self.ng, device=self._device) 
        dreal_dpg[self.spv, :] = torch.eye(self.ng, device=self._device)
        ...
        dreact_dqg = torch.zeros(self.nbus, self.ng, device=self._device)
        dreact_dqg[self.spv, :] = torch.eye(self.ng, device=self._device)
        ...
        jac = torch.cat([
            torch.cat([dreal_dpg.unsqueeze(0).expand(vr.shape[0], *dreal_dpg.shape), 
                torch.zeros(vr.shape[0], self.nbus, self.ng, device=self._device), 
                dreal_dvm, dreal_dva], dim=2),
            torch.cat([torch.zeros(vr.shape[0], self.nbus, self.ng, device=self._device), 
    ...
    def process_output(self, X, out):
        ...
        va = torch.zeros(X.shape[0], self.nbus, device=self._device, dtype=torch.get_default_dtype())
        va[:, self.nonslack_idxes] = out[:, self.va_start_yidx:]
        va[:, self.slack] = torch.tensor(self.slack_va, device=self._device, dtype=torch.get_default_dtype()).unsqueeze(0).expand(X.shape[0], self.nslack)
    ...
    def complete_partial(self, X, Z):
        #print("Z", Z)
        Y_partial = torch.zeros(Z.shape, device=self._device)
